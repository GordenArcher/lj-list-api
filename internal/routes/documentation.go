...
func buildAPIDocumentation() models.APIResponse {
  ...
  -b cookies.txt`,
			},
			{
				...
				Request: map[string]string{
					...
					"landmark":          "string (optional, non-empty)",
					"region":            "string (optional, non-empty)",
    ...
    "password":"password123"
			...
			// Application Endpoints (Customer)
			{
				...
				Request: map[string]any{
					...
					"address":           "string (optional, falls back to profile)",
					"landmark":          "string (optional, falls back to profile)",
					"region":            "string (optional, falls back to profile)",
					"city":              "string (optional, falls back to profile)",
  ...
  -b cookies.txt`,
			...
			},
			{
				Method:      http.MethodPost,
				Path:        "/api/v1/admin/categories",
				Description: "Create a product category (admin only). Category display rules such as Seasonal/inquiry-only can be set once and inherited by every product under that category.",
				Auth:        true,
				AdminOnly:   true,
				Request: map[string]any{
					"name":             "string (required, display name such as 'Rice, Spaghetti & Grains')",
					"description":      "string (optional, customer-facing category/product description)",
					"instructions":     "string (optional, customer-facing instructions)",
					"tag":              "string (optional, inherited product badge such as 'Seasonal')",
					"requires_inquiry": "boolean (optional, defaults to false)",
					"orderable":        "boolean (optional, defaults to true; defaults false when requires_inquiry is true)",
    ...
    "active":true
			...
			},
			{
				...
				Request: map[string]any{
					...
					"tag":              "string (optional)",
					"requires_inquiry": "boolean (optional)",
