...
func Register(router *gin.Engine, pool *pgxpool.Pool, cfg config.Config) {
	...
	packageService := services.NewPackageService(productRepo, productImageRepo, packageRepo, applicationRepo, cfg)
	...
	admin.Use(middleware.AuthRequired(cfg), middleware.AdminRequired)
	{
		...
		admin.PATCH("/packages/fixed/:id/reactivate", packageHandler.ReactivateFixed)
		...
		admin.PATCH("/packages/provisions/:id/reactivate", packageHandler.ReactivateProvisions)
		...
		admin.PATCH("/packages/detergents/:id/reactivate", packageHandler.ReactivateDetergents)
